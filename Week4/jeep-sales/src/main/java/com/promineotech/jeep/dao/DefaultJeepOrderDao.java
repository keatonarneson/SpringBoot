...
public class DefaultJeepOrderDao implements JeepOrderDao {
    ...
    public Order saveOrder(Customer customer, Jeep jeep, Color color, Engine engine, Tire tire, BigDecimal price,
      ...
      return Order.builder().orderPK(orderPK).customer(customer).model(jeep).color(color).engine(engine).tire(tire).options(options).price(price).build();
    }
  
    private void saveOptions(List<Option> options, Long ordersPK) {
      for (Option option : options) {
        SqlParams params = generateInsertSql(option, ordersPK);
